...
This document specifies how to hold a private signing key on macOS so that it can be used without
user interaction, is scoped to a set of binaries that can be rotated freely, and cannot be
extracted from the machine. It states one recommended design, how to build it, and why it meets
those requirements.

Findings are tabulated in Appendix A. The two classes of evidence used — **measured** and
**documented or decoded** — are defined in Appendix B; they are not equivalent, and each claim
below is marked with its basis.
...
**A Secure Enclave key scoped by a team access group.**

1. **Generate the key in the Secure Enclave, persistent, with no presence gate.** The key is then
   non-extractable — no ACL, no password, and no root access yields the material — and every
   operation is unattended. `kSecAccessControlPrivateKeyUsage` may be set; it is not a presence
   requirement (§3.5).
2. **Scope access with `keychain-access-groups`**, declaring a stable group of the form
   `TEAMID.com.example.signing`. The group is anchored to the team, not to a certificate or a
   binary hash, so binaries may be rotated and certificates renewed without touching the key's
   access control (§3.4).
3. **Sign every binary that declares the group with the hardened runtime and library validation.**
   With no per-binary pin, this is the control that prevents code injected into a group-entitled
   binary from inheriting the key, rather than defence in depth (§3.6). Enforce it in the build
   pipeline (§2.5).
4. **Obtain an Apple Developer account.** This design is not reachable without one:
   `keychain-access-groups` is a restricted entitlement and AMFI terminates any binary claiming it
   on a chain it cannot anchor to Apple (§3.2, F9, measured).
5. **Keep the machine inside an organisationally controlled management plane** — MDM enrolment,
   supervision, and a recovery lock. This, and not the key's access control, is what constrains an
   administrator: a supervised device cannot be unenrolled, a recovery lock closes recoveryOS, and
   restrictions can be placed on an administrator who holds `sudo` (§3.7, §2.6, EVIDENCE §6.4).
...
The account grants nothing by itself. It is the only route to a provisioning profile, and a
provisioning profile is the only way to be granted a restricted entitlement (§3.2).

### 1.3 Not the recommendation

Where an Apple Developer account is unavailable, a key in the legacy file keychain can be pinned to
a code requirement instead. That design is documented in [`ALT-ACL-PATH.md`](ALT-ACL-PATH.md) and
is not a variant of this one; nothing in this document depends on it. It is named here so that it
is not mistaken for a fallback.

Three measured properties separate the two, and none of them can be configured away: the key
material stays in a container that root can read, because the pin governs the ACL and not the
container; the ACL always retains a keychain-password prompt alternative that cannot be suppressed;
and the scope it expresses is code rather than a team, so everything finer than a team pin has to be
re-pinned when a certificate is renewed (F15, F16, F17; EVIDENCE §4.3, §9.4). The comparison is in
§3.8.
...
### 2.1 Team, provisioning profile and entitlements

Access is governed by the `keychain-access-groups` entitlement rather than by any code requirement.
A provisioning profile is Apple's signed authorization binding a team, a set of certificates and a
set of entitlements. It grants a **prefix**; the binary declares concrete groups beneath it. Decoded
...
The binary must carry the matching `keychain-access-groups` entitlement, and its profile must grant
the prefix. AMFI verifies three conditions at launch: the signature chains to Apple, the signing
certificate appears in `DeveloperCertificates`, and the profile grants the entitlement being
claimed (§3.2).

The signing leaf is a Developer ID certificate. Its private key remains under local control; Apple
never has access to it. Decoding an installed application shows the shape (`run-profile.sh`):
...
The certificate is not what scopes the key. Obtaining a Developer ID certificate is what makes the
profile possible; the certificate on its own carries no right to the key.

### 2.2 Creating the Secure Enclave key
...
The key must be created on the machine that will hold it: SE keys are per-machine and
non-portable. That is the security property the design relies on, and it also means a host that
dies takes its key with it, so provisioning must allow for N+1 capacity or a rebuild path.

### 2.3 Hardware assertions at startup

An SE request can silently degrade: asking for a persistent key while targeting the legacy keychain
produces a software key, with no error at all (F9 note, EVIDENCE §8.2):
...
A genuine SE key reports `tkid = com.apple.setoken` and refuses export (`extr = 0`). Assert both
properties in the signer's startup path, or the downgrade is invisible. Both belong on this path
because the keychain is not optional (§2.4).

### 2.4 Keychain selection

An SE key belongs in the **data-protection keychain**. `kSecAttrAccess`, the attribute that carries
a CSSM ACL, is a legacy file-keychain attribute; the modern path scopes items by entitlement access
group instead (F19). That is the property this design depends on:

- **No ACL, therefore no prompt** (§3.5).
- **Scoped to the team**, matching §2.1.
- **The material never leaves the SEP**, so there is nothing for a container-level read to recover.
- **The account is a precondition**, because the entitlement is restricted (§3.2).

An item requested as persistent against the legacy file keychain degrades silently to a software
key (§2.3), which is why both hardware assertions are required.

### 2.5 Signing flags

Sign every binary that declares the access group with the hardened runtime and library validation:

```
codesign --options runtime --sign "Developer ID Application: ..." <binary>
```

Neither `com.apple.security.cs.allow-dyld-environment-variables` nor
`...disable-library-validation` may be added. Enforce the flags in the build pipeline, because
there is no per-binary pin behind them on this path (§3.6).

### 2.6 The host machine

The machine that holds the key must be enrolled in an organisationally controlled management plane.
...
The first row determines the outcome: removing an endpoint agent or changing a local configuration
achieves nothing durable, because the device remains managed. "The user has `sudo`" and "the user
can leave the management plane" are different statements.
...
### 2.7 Verification order

Run once an Apple Developer account exists. Items 1–5 are the unverified properties in §3.9.
...
### 3.1 Requirement and mechanism

| requirement | mechanism | basis |
|---|---|---|
| signs with no user interaction | SE key with no presence gate | measured, F18 (`run-se.sh`) |
| key material cannot be extracted | generated in the SEP, non-exportable | measured on an ephemeral key, F9 note |
| scope survives binary rotation and certificate renewal | `keychain-access-groups`, anchored to the team | decoded profiles, §3.4 |
| a foreign team cannot reach the key | restricted entitlement granted only by that team's profile | decoded profiles and SDK documentation |
| code injected into a permitted binary cannot reach the key | hardened runtime with library validation | measured, F7 (`run-injection.sh`) |
| a local administrator cannot reach the key | non-extractability, plus the management plane | F9 measured; §3.7 documented |

### 3.2 The Apple Developer account is a precondition

The account is a precondition, not an option, and it is the only reason it is needed: the account
itself grants nothing, but it is the only route to a provisioning profile, and a provisioning
profile is the only way to be granted a restricted entitlement.
...
AMFI's stated reason is `Unable to retrieve certificate chain`. Adding the CA to the user trust
store does not alter this, because AMFI's anchor set is not the user's trust settings.

The dividing line is therefore not "signed versus unsigned" but **restricted versus unrestricted
entitlements**. `keychain-access-groups` is restricted, which is why the account is a precondition
(§1.1 step 4). A design that claims no restricted entitlement can be built without one; that is the
alternative in [`ALT-ACL-PATH.md`](ALT-ACL-PATH.md).

### 3.3 The chain is Apple's; there is no private CA
...
- **The chain is Apple's, so the restricted entitlement is accepted.** AMFI validates the
  Apple-anchored chain, the signing certificate and the profile together (§2.1).
- **The scope is team-level and stable.** The access group is anchored to the team, so reissuing
  the leaf, or rebuilding the binary, changes nothing about which binaries reach the key (§3.4).
...
### 3.4 The scope is the team

The group string is chosen by the team and is stable. Two consequences follow, and both are the
properties the design depends on:

- **A rebuilt binary declaring the same group reaches the same key.** No re-pinning is required, at
  any rotation frequency.
- **Cross-team access is refused.** A binary from another team cannot declare that prefix, because
  the prefix is not authorised by *its* profile.

The scope is the **team**, not a binary or a certificate hash. This is intentional: binary rotation
requires no action, at the cost of per-binary granularity. The group is invalidated only by leaving
the team. The alternative's pin forms, which are invalidated by certificate renewal or by
rebuilding, are tabulated in `ALT-ACL-PATH.md` §3.

> **Basis.** This subsection is from decoded provisioning profiles and SDK documentation, not from
> measurement: a persistent data-protection item cannot be created without the entitlement (F9).
> §2.7 records the specific test to run once an account exists.

### 3.5 No presence gate, no ACL, no prompt

An SE key requires no user interaction unless a presence gate is requested. Only these flags impose
one:
...
The presence gates exist for the opposite use case — SE keys backing operations a human authorises,
such as payments or unlocking a credential store — where requiring proof of presence is the point.
They are capabilities rather than requirements, and an unattended signer does not select them.

The data-protection keychain adds no prompt of its own: it is scoped by entitlement rather than
CSSM ACL, so no ACL entry exists and no `KeychainPromptAclSubject` is present behind the key (§2.4).

### 3.6 Hardened runtime is the primary control

Signatures do not cover `dlopen`ed libraries. Measured:
...
lets anything it loads use the key. Enforce the flags in the build pipeline (§1.1 step 3, §2.5).

### 3.7 The management plane bounds an administrator
...
scope is the machine itself, and this is why §1.1 step 5 is not optional — supervision, restrictions
and a recovery lock are the controls that bound an administrator, not the key's access control.

The controls are tabulated in §2.6. The alternative path is defeated by a local administrator
wherever this plane is absent, which is one of the reasons it is not the recommendation (F4, F14;
`ALT-ACL-PATH.md` §5.3).

### 3.8 The alternative path

The alternative — a keychain ACL pinned to a code requirement — carries three exposures that this
design does not, and none can be configured away:

1. **The material stays in a container that root can read.** The pin governs the ACL, not the
   container, so export is refused while the container still opens for anyone who can open it
   (F16).
2. **The ACL always retains a prompt alternative that cannot be suppressed.** The entry is
   `(requirement matches) OR (user approves)`, and no "never prompt" flag exists. A
   prompt-free ACL can be constructed, but the keychain service re-derives a `cdhash` entry for
   the creating process on write (F3, F17).
3. **The scope it expresses is code, not a team.** Everything finer than a team pin has to be
   re-pinned when a certificate is renewed (F15).

Two controls from this design are required on that path as well, and one of them carries more
weight there: hardened runtime is defence in depth behind the pin, and the management plane is the
only control that bounds an administrator at all. What the ACL path offers in exchange is per-binary
granularity and independence from an Apple account.

For an unattended signer that accepts team-level granularity, this design removes all three
exposures. That is why it is the recommendation. The alternative remains the answer when an account
is unavailable, and is developed in [`ALT-ACL-PATH.md`](ALT-ACL-PATH.md).
...
Apple Developer account; the items specific to the alternative are in `ALT-ACL-PATH.md` §6. The
test order that resolves them is §2.7.

1. **Team-group scoping, end to end.** §3.4 rests on decoded provisioning profiles and SDK
...
3. **Non-extractability of a persistent SE key.** Measured only on an *ephemeral* SE key (F9
   note): genuine SE keys report `tkid = com.apple.setoken`, refuse
...
The `path` column says which design a finding bears on.
...
The findings that shape this design are:
...
- **Documented or decoded** — taken from vendor documentation or decoded from real artifacts such
  as provisioning profiles, because the behaviour could not be exercised without an Apple
  Developer account.

All measurements were taken on macOS 26.6.2 (build 25G83), arm64, OpenSSL 3.6.4, command line tools
only, none requiring `sudo`.

**Measured locally.** Everything in `EVIDENCE.md` marked `(measured)`, on macOS 26.6.2 (build
25G83), arm64: keychain ACL behaviour, code-requirement evaluation, entitlement rejection by AMFI,
code injection, ACL prompt behaviour, key extraction, and the permission bits and authorization
rules governing the System keychain.

**Decoded from artifacts.** The provisioning-profile structure in §2.1 and §3.4 was extracted from
installed applications with `run-profile.sh` and `security cms -D`. The payload and trust-setting
behaviour in F6 and F20 was decoded from the system binaries that implement it — `/usr/libexec/trustd`
for the entitlement that gates a trust-settings write, and
`ConfigurationProfiles.framework/XPCServices/CertificateService.xpc` for the payload keys, the trust
values applied, and the ACL-construction gap described in `ALT-ACL-PATH.md` §5.1.

**Documented by the vendor.** The claims in §2.6 and §3.7 come from vendor documentation rather than
measurement, because the machine used for this work is not MDM-enrolled
(`profiles status -type enrollment` reports "MDM enrollment: No") and cannot exhibit the behaviour
described.
...
documentation pages, so the quotations in §2.6 can be re-derived rather than taken on trust.
...
| document | contents |
|---|---|
| `README.md` | the recommended design, its implementation, and the reasoning behind it |
...
| `run-*.sh` | the reproduction scripts, tabulated in Appendix A |
